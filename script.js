...
async function getCastCredits(url)
{
    ...
    for (let i = 0; i < 3; i++) {
        if (respData.cast[i] !== undefined){
            var actorName = respData.cast[i].name;
            if (i === 2)
...
async function showShowInfo(url)
{
    ...
    (async ()=>
    {
        ...
        {
            var castList = '';
            const itemCast = document.querySelector(".item-cast");
            const castResp = await fetch("https://api.themoviedb.org/3/tv/"+id+"/credits?api_key="+APIKEY);
            const castData = await castResp.json();

            castList = "Cast: <a class='cast' value="+castData.cast[0].id+">" + castData.cast[0].name + "</a>, <a class='cast' value="+castData.cast[1].id+">" + castData.cast[1].name + "</a> & <a class='cast' value="+castData.cast[2].id+">" + castData.cast[2].name + "</a>.";

            itemCast.innerHTML = `${castList}`;

            const itemCasts = document.querySelectorAll(".item-cast .cast");

            itemCasts.forEach(itemCastName => {
                itemCastName.onclick = ()=>
                {
                    getPerson("https://api.themoviedb.org/3/person/"+itemCastName.getAttribute('value')+"?api_key="+APIKEY);
                }
            });
        })();
...
async function showMovieInfo(url)
{
    ...
    (async ()=>
    {
        ...
        (async ()=>
        {
            var castList = '';
            const itemCast = document.querySelector(".item-cast");
            const castResp = await fetch("https://api.themoviedb.org/3/movie/"+id+"/credits?api_key="+APIKEY);
            const castData = await castResp.json();
...
{
    const personResp = await fetch(url);
    const personData = await personResp.json();

    const {profile_path, name, place_of_birth, birthday, id, biography} = personData;
    ...
    personContainer.innerHTML = `<div class="person-details">
                                    <span class="material-icons-outlined close" style="font-size:30px;">close</span>
                                    <img src="${IMGPATH + profile_path}" alt="${name}" class="person-profile-pic">
                                    <div class="person-name-details">
                                        <div class="person-name">Name: ${name}</div>
                                        <div class="person-date-place">
                                            <div class="person-date">Date of Birth: ${birthday}</div>
                                            <div class="person-place">Place of Birth: ${place_of_birth}</div>
                                        </div>
                                    </div>
                                    </div>
                                ...
                                <div class="person-movies">
                                    <label for="person-movies-label" class="person-movies-label">Movies:</label>
                                    <div class="movies">

                                    </div>
                                </div>
                                <div class="person-shows">
                                    <label for="person-shows-label" class="person-shows-label">TV Shows:</label>
                                    <div class="shows">

                                    </div>
                                </div>`;

    const movies = document.querySelector(".person-movies .movies");

    (async () => 
    {
        const personMovieResp = await fetch("https://api.themoviedb.org/3/person/"+id+"/movie_credits?api_key=6b0c058c1cf67505e91990e96cbe81f5");
        const personMovieData = await personMovieResp.json();

        personMovieData.cast.forEach(personMovie => 
        {
            const {poster_path, id, title, vote_average} = personMovie;
            ...
            if (poster_path !== null)
            {
                (async ()=>
                {
                    ...
                    movie.innerHTML = `<img class="person-movie-poster" src="${IMGPATH + poster_path}" alt="${title}">
                                        <div class="person-movie-details">
                                            <div class="movie-name-cast">
                                                <p class="movie-name">
                                                    ${title}
                                                </p>
    ...
    const shows = document.querySelector(".person-shows .shows");

    (async () => 
    {
        const personShowResp = await fetch("https://api.themoviedb.org/3/person/"+id+"/tv_credits?api_key=6b0c058c1cf67505e91990e96cbe81f5");
        const personShowData = await personShowResp.json();

        personShowData.cast.forEach(personShow => 
        {
            const {poster_path, id, name, vote_average} = personShow;
            ...
            if (poster_path !== null)
            {
                (async ()=>
                {
                    ...
                    show.innerHTML = `<img class="person-show-poster" src="${IMGPATH + poster_path}" alt="${name}">
                                        <div class="person-show-details">
                                            <div class="movie-name-cast">
                                                <p class="movie-name">
                                                    ${name}
                                                </p>
    ...
    const closeBtn = document.querySelector(".person-container .close");
    closeBtn.onclick = ()=>
    {
        wrapper.classList.remove('active');
        containerBG.classList.remove('active');
